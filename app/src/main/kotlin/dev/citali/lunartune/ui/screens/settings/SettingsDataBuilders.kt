...
fun buildSettingsGroups(
    ...
    val integration =
        SettingsItem(
            ...
            children = listOf(
                SettingsChild("Discord Integration", "discord", listOf("discord", "rich presence", "rpc", "status", "now playing")) { SearchResultSwitch(EnableDiscordRPCKey, false) },
                SettingsChild("Last.fm / Libre.fm Integration", "lastfm", listOf("lastfm", "last.fm", "libre.fm", "scrobble", "scrobbling")),
                SettingsChild("ListenBrainz scrobbling", "listenbrainz", listOf("listenbrainz", "listen brainz", "scrobble")) { SearchResultSwitch(ListenBrainzEnabledKey, false) },
                SettingsChild("Set ListenBrainz token", "listenbrainz_token", listOf("listenbrainz token", "listenbrainz api key")),
                SettingsChild("Connect Spotify", "spotify", listOf("spotify", "spotify connect", "spotify playlists", "spotify login")) { SearchResultSwitch(ShowSpotifyPlaylistsKey, false) },
            ),
        )
    val aiIntegration =
        SettingsItem(
